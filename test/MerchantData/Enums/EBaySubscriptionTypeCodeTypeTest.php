<?php
/**
 * DO NOT EDIT THIS FILE!
 *
 * This file was automatically generated from external sources.
 *
 * Any manual change here will be lost the next time the SDK
 * is updated. You've been warned!
 */

namespace LT\eBaySDK\Test\MerchantData\Enums;

use LT\eBaySDK\MerchantData\Enums\EBaySubscriptionTypeCodeType;

class EBaySubscriptionTypeCodeTypeTest extends \PHPUnit_Framework_TestCase
{
    private $obj;

    protected function setUp()
    {
        $this->obj = new EBaySubscriptionTypeCodeType();
    }

    public function testCanBeCreated()
    {
        $this->assertInstanceOf('\LT\eBaySDK\MerchantData\Enums\EBaySubscriptionTypeCodeType', $this->obj);
    }
}
