...
The ``LT\eBaySDK\MerchantData\Services\MerchantDataService`` class was removed in version 1 and replaced with the ``LT\eBaySDK\MerchantData\MerchantData`` class. There is no difference in how the new class works when compared with the previous version.

Removed UnboundType class
~~~~~~~~~~~~~~~~~~~~~~~~~

The ``LT\eBaySDK\Types\UnboundType`` class was removed in version 1 and replaced with the ``LT\eBaySDK\Types\RepeatableType`` class. There is no difference in how the new class works when compared with the previous version.
...
.. code-block:: php
    ...
    use LT\eBaySDK\Trading\Services\TradingService;
...
The pre-version 1 SDK would throw a ``LT\eBaySDK\Exceptions\UnknownConfigurationOptionException`` when a non-existent configuration option was passed to either the service constructor or ``config`` method. The SDK will now silently ignore any unknown configuration options.
...
.. code-block:: php
    ...
    } catch (\LT\eBaySDK\Exceptions\UnknownConfigurationOptionException $e) {
        echo $e->getMessage();
    }
...
.. code-block:: php
    ...
    // Pre-version 1
    class HttpClient implements \LT\eBaySDK\Interfaces\HttpClientInterface
    {
