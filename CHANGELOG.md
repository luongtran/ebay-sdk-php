...
* SDK now ignores properties in the JSON response that would normally trigger an LT\eBaySDK\Exceptions\UnknownPropertyException exception.
...
* Removed constant LT\eBaySDK\Marketing\Enuns\InventoryCriterionEnum::C_INVALID.
* Removed property inventoryReferenceType from LT\eBaySDK\Marketing\TypesInventoryItem class.
...
* Allow properties on `LT\eBaySDK\Types\DecimalType` objects to be assigned both `integer` and `double` values.
...
* The protected method `\LT\eBaySDK\Services::BaseService::callOperation` has been removed. Code that called this method will now need to call `callOperationAsync(...)->wait()` instead.
...
* The class `\LT\eBaySDK\Handler` is now `\LT\eBaySDK\HttpHandler`.
...
* Some properties where incorrectly declared as being `LT\eBaySDK\ResolutionCaseManagement\Types\Integer` instead of `integer`.
