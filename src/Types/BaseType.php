...
use \LT\eBaySDK\Types;
use \LT\eBaySDK\Exceptions;
use \LT\eBaySDK\JmesPath\Env;
use \LT\eBaySDK\JmesPath\JmesPathableObjectInterface;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \LT\eBaySDK\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \LT\eBaySDK\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \LT\eBaySDK\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'LT\eBaySDK\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \LT\eBaySDK\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\LT\eBaySDK\Types\BaseType', false)) {
