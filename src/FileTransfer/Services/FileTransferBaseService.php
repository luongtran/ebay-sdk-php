 ...
 */
class FileTransferBaseService extends \LT\eBaySDK\Services\BaseService
{
    /**
    ...
    public static function getConfigDefinitions()
    {
        ...
        return $definitions + [
            'apiVersion' => [
                'valid' => ['string'],
                'default' => \LT\eBaySDK\FileTransfer\Services\FileTransferService::API_VERSION
            ],
            'authToken' => [
                'valid' => ['string'],
                'required' => true
    ...
    protected function callOperationAsync($name, \LT\eBaySDK\Types\BaseType $request, $responseClass)
    {
        ...
        if ($name === 'uploadFile' && $request->hasAttachment()) {
            ...
            if (!isset($request->fileAttachment)) {
                $request->fileAttachment = new \LT\eBaySDK\FileTransfer\Types\FileAttachment();
            }

            if (!isset($request->fileAttachment->Data)) {
                $request->fileAttachment->Data = new \LT\eBaySDK\FileTransfer\Types\Data([
                    'xopInclude' => new \LT\eBaySDK\FileTransfer\Types\XopInclude([
